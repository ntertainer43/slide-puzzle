...
class AI:
    ...
    def move(self,a):
        b = play(WIN, self.player1, self.player2, a)
        print(b)
        i = 1
##        valid_moves =[]
##        valid_boxes = []
...
##            print(set(permu),set(self.opp), wise_move, len(wise_move))
    ...
    def hasWon(self, list_of_boxes):
        ...
        for mark, ind in box_pos:
            if mark == 1:
                self.player1.append(ind)
            elif mark == -1:
                self.player2.append(ind)
        self.opp = self.player1
        for permu in WIN:
            if set(permu) <set(self.player1) or set(permu)== set(self.player1):
                gameWinAnimation("Player 1")
                return False
            elif set(permu) <set(self.player2) or set(permu) == set(self.player2):
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                #keeps track of players when valid mouse button is pressed
                   ...
                   if box.latest == True:
                       a = (box.position[1]-1)*3 +box.position[0]
                       print(a)
                       huTurn = True
...
##        print(player)
        if player == -1 and NotWin and not huTurn:
            player =random_bot.move(a)
...
def play(WIN, player1, player2,a):
##    player1 =[]
##    player2 =[]
   oldwise= [0]
   oddlist=[1,3,7,9]
...
##        player1.append(a)
   if len(player1) == 0:
      ...
      return b
   if len(player1) == 1 and a == 5:
        random.shuffle(oddlist)
        print(oddlist)
        b = oddlist[0]
        print(b)
   elif len(player1) == 1 and a%2 in (1,0):
        b = 5
   else:
        for permu in WIN:
            wise_move = set(permu)- set(player1)
            print(wise_move)
            if len(wise_move) == 1 and list(wise_move)[0] not in player2:
                b = wise_move.pop()
                break
            else:
                for mov in oldwise:
                    if mov in wise_move and not mov in player2:
                        b = mov
                oldwise = wise_move
##   print("The bot moved", b)
   
   try:   
      return b
   except UnboundLocalError as e:
      print(e)
      return ({1,2,3,4,5,6,7,8,9}-set(player1))-set(player2)
##        player2.append(b)
